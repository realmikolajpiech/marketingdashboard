  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end">
      ...
      <motion.aside
        ...
        className="relative w-full max-w-md h-dvh bg-white shadow-2xl flex flex-col ring-1 ring-stone-200"
      >
        <header className="px-5 py-4 border-b border-stone-100 flex items-start justify-between gap-3">
          ...
          <button
            onClick={onClose}
            className="p-2 rounded-lg text-stone-400 hover:text-stone-600 hover:bg-stone-100 transition-colors shrink-0"
          >
            <X className="w-4 h-4" />
          </button>
        </header>
        ...
        <div className="flex-1 overflow-y-auto scrollbar-thin">
          {editing ? (
            ...
            >
              <div>
                ...
                <input
                  ...
                  required
                  className="w-full px-3 py-2 text-sm border border-stone-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500"
                />
              </div>

              <div>
                <label className="block text-xs font-medium text-stone-600 mb-1">Platforms</label>
                <PlatformPicker
              ...
              <div className="space-y-3">
                {platformProfiles.map((profile) => (
                  <PlatformProfileEditor
                    ...
                    inputClass="w-full px-3 py-2 text-sm border border-stone-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500"
                  />
                ))}
              </div>
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full px-3 py-2 text-sm border border-stone-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500/20"
                >
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={4}
                  className="w-full px-3 py-2 text-sm border border-stone-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500/20 resize-none"
            ...
            <div className="p-5 space-y-6">
              {suggested && (
                <div className="rounded-xl bg-teal-50 ring-1 ring-teal-200 px-4 py-3.5">
                  <p className="text-xs font-medium text-teal-800">Suggested price per video</p>
                  <p className="text-xl font-semibold text-teal-900 tabular-nums font-mono mt-0.5">
              ...
              <div className="space-y-2">
                <p className="text-xs font-medium text-stone-500">Platforms</p>
                {displayProfiles.map((profile) => {
                  ...
                  return (
                    <div
                      key={profile.platform}
                      className="rounded-xl ring-1 ring-stone-200/80 px-3.5 py-3 space-y-2"
                    >
                      <div className="flex items-center justify-between gap-2">
                        <div className="flex items-center gap-2 min-w-0">
                          <PlatformIcon platform={profile.platform} size="sm" href={profileUrl} />
                          <div className="min-w-0">
                            <p className="text-sm font-medium text-stone-900">{profile.platform}</p>
                            <p className="text-xs text-stone-500 truncate">{profile.handle}</p>
                          </div>
                        </div>
                        {profileUrl && (
                          <a
                      ...
                      <div className="grid grid-cols-2 gap-2 text-xs">
                        <div className="rounded-lg bg-stone-50 px-2.5 py-2">
                          ...
                          <p className="font-semibold text-stone-900 tabular-nums font-mono mt-0.5">
                            {formatCompact(profile.followers)}
                          </p>
                        </div>
                        <div className="rounded-lg bg-stone-50 px-2.5 py-2">
                          <p className="text-stone-500">Avg views</p>
                          <p className="font-semibold text-stone-900 tabular-nums font-mono mt-0.5">
                      ...
                      {profileSuggested && (
                        <p className="text-xs text-teal-700 font-medium tabular-nums font-mono">
              ...
              <div className="grid grid-cols-3 gap-3">
                {[
                  ...
                  <div key={item.label} className="rounded-lg bg-stone-50 px-3 py-2.5">
                    <p className="text-[11px] text-stone-500">{item.label}</p>
                    <p className={`text-sm font-semibold mt-0.5 tabular-nums font-mono ${item.className ?? "text-stone-900"}`}>
                      {item.value}
                    </p>
                  </div>
                ))}
              ...
              {creator.notes && (
                <div>
                  ...
                  <p className="text-sm text-stone-700 leading-relaxed">{creator.notes}</p>
                </div>
              )}
              ...
              <div>
                ...
                {creatorPayments.length === 0 ? (
                  <p className="text-sm text-stone-400 py-4 text-center bg-stone-50 rounded-lg">
                  ...
                  <div className="space-y-2">
                    {creatorPayments.map((p) => (
                      <div
                        key={p.id}
                        className="flex items-start justify-between gap-3 p-3 rounded-lg ring-1 ring-stone-200/80 group"
                      >
                        <div className="min-w-0">
                          <p className="text-sm font-semibold text-stone-900 tabular-nums font-mono">
                            {formatCurrency(p.amount)}
                          </p>
                          <p className="text-xs text-stone-400 mt-0.5">{p.paymentDate}</p>
                          {p.notes && (
                            <p className="text-xs text-stone-500 mt-1">{p.notes}</p>
                          )}
                          {p.videoUrl && (
                            <a
                              href={p.videoUrl}
                              target="_blank"
                              rel="noreferrer"
                              className="inline-flex items-center gap-1 mt-1.5 text-xs font-medium text-teal-700"
                            >
                        ...
                        <button
                          ...
                          className="p-1 rounded text-stone-300 hover:text-rose-600 opacity-0 group-hover:opacity-100 transition-all"
                        >
        ...
        {!editing && (
          <footer className="px-5 py-4 border-t border-stone-100 flex gap-2">
            <button
              onClick={startEdit}
              className="flex-1 px-4 py-2.5 text-sm font-medium text-stone-700 bg-stone-100 hover:bg-stone-200 rounded-lg transition-colors flex items-center justify-center gap-1.5"
            >
            ...
            <button
              ...
              className="px-4 py-2.5 text-sm font-medium text-rose-600 hover:bg-rose-50 rounded-lg transition-colors"
            >
              Delete
            </button>
          </footer>
