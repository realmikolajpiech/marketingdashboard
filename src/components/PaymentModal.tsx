  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
      ...
      <motion.div
        ...
        exit={{ opacity: 0, y: 24 }}
        className="relative w-full sm:max-w-md bg-white rounded-t-2xl sm:rounded-2xl shadow-xl ring-1 ring-stone-200"
      >
        <div className="flex items-center justify-between px-5 py-4 border-b border-stone-100">
          <div>
            <h2 className="text-base font-semibold text-stone-900">Record payment</h2>
            <p className="text-xs text-stone-500 mt-0.5">Log money paid to a creator</p>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          <div>
            <label className="block text-xs font-medium text-stone-600 mb-1.5">Creator</label>
            <select
              required
              value={creatorId}
          ...
          <div>
            <label className="block text-xs font-medium text-stone-600 mb-1.5">
              Video link <span className="text-stone-400 font-normal">(optional)</span>
            </label>
            <input
              type="url"
              placeholder="https://..."
